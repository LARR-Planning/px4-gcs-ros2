...
px4_gcs::Px4GcsWrapper::Px4GcsWrapper(const rclcpp::NodeOptions &options_input) : Node("px4_gcs_wrapper",
    ...
    {   // subscriber group
        ...
        current_pose_external_subscriber_ = create_subscription<LocalPoseExternal>("~/current_external_pose",
                                                                                   rclcpp::QoS(1), std::bind(
                        &Px4GcsWrapper::ExternalPoseCallback, this, std::placeholders::_1), options);
        options.callback_group = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        current_pose_rtps_subscriber_ = create_subscription<LocalPoseRtps>("~/current_rtps_pose", rclcpp::QoS(1),
                                                                           std::bind(
                                                                                   &Px4GcsWrapper::RtpsPoseCallback,
    ...
    if (is_tf_received_ and is_rtps_pose_received_) {
        ...
        if (abs(pose_rtps_.pose.position.x) < 1e4) {
            ...
            RCLCPP_INFO_STREAM_ONCE(get_logger(),
                                    "[PX4 GCS Wrapper] Initializing the homing point with the current pose.");
            return true;
        } else {
            is_init_mav_ = false;
            RCLCPP_INFO_STREAM_ONCE(get_logger(),
                                    "[PX4 GCS Wrapper] Tried to initialize homing point from rtps local position. But the value sanity check (x<1e4) failed.");
        }
    } else {
